...
if haveProg apt-get; then
  ...
  LINUX=1
  PKGMGR=4
elif haveProg pacaur; then
  echo "You are using pacaur. I'll assume you have Linux with that."
  LINUX=1
  PKGMGR=6
else
  MAC=1
...
PACKAGES_PACMAN_LINUX='optipng libjpeg phantomjs'
DEBGIT='git-core'
OTHERGIT='git'
...
elif [ -z "$RUBYFILE" ] && [ "$LINUX" -eq 1 ] && [ "$PKGMGR" -eq 6 ]; then
  echo "Installing Ruby"
  pacaur -S ruby rubygems
...
if [ "$LINUX" -eq 1 ]; then
  ...
  elif [ "$PKGMGR" -eq 6 ]; then	
    sudo pacaur -S $PACKAGES_PACMAN_LINUX $OTHERGIT
  fi
fi
